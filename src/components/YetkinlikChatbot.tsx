...
export const YetkinlikChatbot = ({
  ...
  const getPresetMessages = () => [
    ...
    i18n.language === 'tr' 
      ...
      : `*Basic Robot Maintenance Knowledge (Brand Independent)*\n\nExplain the difference between 'preventive maintenance' and 'corrective maintenance' in an industrial robot.\nExplain with a real example.`
  ];
      ...
      <div className={cn(
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-border bg-muted/30">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base font-semibold text-foreground">{t('chatbot.title')}</h2>
        ...
        <div className="relative flex-1 overflow-y-auto p-6 space-y-4 min-h-[300px] bg-gradient-to-b from-muted/10 to-transparent">
          {messages.map(message => (
            <div 
              key={message.id} 
              className={cn(
                "max-w-[85%] p-4 rounded-2xl rounded-tl-md",
          ...
          {isTyping && (
            <div className={cn(
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-muted-foreground text-xs">{t('chatbot.typing')}</span>
              </div>
            </div>
          )}
...
export const FuturisticVerifyButton = ({
  ...
  return (
    <div className="mt-8 p-6 rounded-2xl bg-gradient-to-br from-muted/50 to-muted/20 border border-border">
      <h3 className="text-lg font-semibold text-foreground mb-4 flex items-center gap-2">
        <Zap className="w-5 h-5 text-primary" />
        {t('entegratorRegister.verifyCompetency')}
      </h3>
      ...
      >
        ...
        <span className="relative flex items-center justify-center gap-3">
          ...
          {t('entegratorRegister.verifyWithAI')}
          <ArrowRight className="w-4 h-4 opacity-0 -translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-300" />
        </span>
      </button>
      
      <p className="text-sm text-muted-foreground mt-3 text-center">
        {t('entegratorRegister.personalizedAI')}
      </p>
    </div>
  );
};

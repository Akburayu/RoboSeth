...
export function EntegratorEslesmeler() {
  ...
  if (matches.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center">
        ...
        <h2 className="text-2xl font-bold text-foreground">Açık Projeniz Bulunmuyor</h2>
        <p className="text-muted-foreground mt-2 max-w-md">
          Firmalar tarafından henüz bir eşleşme talebi veya proje daveti almadınız. Davetler burada belirecektir.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col md:flex-row gap-6">
      {/* Yan Liste - Proje Sekmeleri */}
      <div className="w-full md:w-64 shrink-0 space-y-2">
        <h3 className="font-semibold text-sm uppercase tracking-wider text-muted-foreground mb-4">
          Davet Edildiğiniz Projeler
        </h3>
        {matches.map(m => (
          <Button
            key={m.id}
      ...
      {/* Seçili Proje - İş Akışı */}
      <div className="flex-1 bg-card rounded-lg border shadow-sm p-6 max-w-4xl">
        {selectedMatch ? (
          <>
            <div className="mb-8 border-b pb-4">
              <h2 className="text-2xl font-bold text-foreground flex items-center gap-2">
                <Building2 className="text-primary" /> {selectedMatch.projectName}
              </h2>
              <p className="text-muted-foreground">İşveren ID: {selectedMatch.firmaId.substring(0, 8)}...</p>
            </div>
